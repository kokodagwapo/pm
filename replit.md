...
## Google Maps Integration (Google Maps ONLY — no Leaflet/OpenStreetMap)
...
- **RentalsGoogleMap** (`src/components/landing/RentalsGoogleMap.tsx`): Google Maps only, uses `@react-google-maps/api` (`GoogleMap`, `MarkerF`, `useJsApiLoader`). Supports roadmap/satellite/terrain/street view modes. Auto-fits bounds when properties change. Shows placeholder when API key is missing or load fails. Shows warning banner on auth failure (`gm_authFailure`). **No Leaflet/OpenStreetMap fallback.**
- **SinglePropertyMap** (`src/components/landing/SinglePropertyMap.tsx`): Single-property map with one marker using vanilla Google Maps JS API.
- **MapErrorBoundary** (`src/components/landing/MapErrorBoundary.tsx`): React class error boundary wrapping all map components.
- **CRITICAL**: The `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` must have the production domain whitelisted in Google Cloud Console. The Replit dev domain will show `RefererNotAllowedMapError` unless also whitelisted.
