...
- **Global error boundary**: `global-error.tsx` — empty errors (no message) render a blank `<body/>` with zero hooks/effects. Real errors show a simple centered card with "Try Again" / "Go Home" buttons. No `window.location.reload()` — avoids infinite refresh loops.
- **Dev overlay suppressor**: `layout.tsx` injects a dev-only `<script>` (MutationObserver) that watches for `<nextjs-portal>` elements. When the portal contains "Critical Application Error" + "{}", it hides the overlay so the app preview remains visible. Capped at 3 suppressions per session via `sessionStorage`. Production builds never include this script.
