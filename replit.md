...
- **Window-level webpack error suppression**: `layout.tsx` includes a dev-only `<script>` that intercepts `options.factory` / `webpack_require` errors at the DOM level before they propagate. Gated on `process.env.NODE_ENV === 'development'` and narrowed to webpack stack traces only — production never runs this code.
