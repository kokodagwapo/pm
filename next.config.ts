    ...
    if (dev && !isServer) {
      config.plugins.push(
        new webpack.BannerPlugin({
          ...
          banner: `(function(){if(typeof window==='undefined'||window.__wpErrGuard)return;window.__wpErrGuard=true;function isEmpty(err){if(!err)return true;if(typeof err==='object'&&!(err instanceof Error)){try{if(Object.keys(err).length===0)return true}catch(_){}}return false;}function isTransient(err){if(!err)return true;if(isEmpty(err))return true;var s=String(err.stack||'');var m=String(err.message||'');if(s.indexOf('options.factory')!==-1)return true;if(s.indexOf('webpack_require')!==-1)return true;if(m.indexOf("reading 'call'")!==-1&&m.indexOf('undefined')!==-1)return true;if(m.indexOf('ChunkLoadError')!==-1)return true;if(m.indexOf('Loading chunk')!==-1)return true;return false;}function bumpReload(){try{var k='__wp_chunk_reload';var n=parseInt(sessionStorage.getItem(k)||'0',10);if(n<2){sessionStorage.setItem(k,String(n+1));location.reload();}}catch(_){}}window.addEventListener('error',function(e){var err=e&&e.error;if(isTransient(err)){e.preventDefault();e.stopImmediatePropagation();if(!isEmpty(err))bumpReload();return false;}},true);window.addEventListener('unhandledrejection',function(e){var r=e&&e.reason;if(isTransient(r)){e.preventDefault();if(!isEmpty(r))bumpReload();}});})();`,
        })
      );
    }
    return config;
