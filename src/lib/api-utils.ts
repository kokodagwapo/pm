...
export async function getOwnerPropertyAccess(userId: string): Promise<OwnerAccessResult> {
  try {
    const Property = (await import("@/models")).Property;
    const properties = await Property.find({ ownerId: userId }).select("_id").lean();
    return {
      isOwner: properties.length > 0,
      ownedPropertyIds: properties.map((p: any) => p._id.toString()),
    };
  } catch (error) {
    console.error("Error getting owner property access:", error);
    return { isOwner: false, ownedPropertyIds: [] };
  }
}

export function buildOwnerPropertyFilter(
  userRole: UserRole,
  userId: string,
  ownedPropertyIds: string[]
): Record<string, any> {
