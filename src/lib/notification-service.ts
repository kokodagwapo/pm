/**
 * SmartStartPM - Comprehensive Notification Service
...
export class NotificationService {
  ...
  private generateSMSMessage(data: NotificationData): string {
    switch (data.type) {
      case NotificationType.PAYMENT_OVERDUE:
        return `SmartStartPM: Your rent payment is overdue. Amount: ${formatCurrency(Number(data.data?.amount))}. Please pay immediately to avoid late fees.`;
      case NotificationType.MAINTENANCE_EMERGENCY:
        return `SmartStartPM EMERGENCY: ${data.data?.description}. Request #${data.data?.requestId}. Immediate attention required.`;
      case NotificationType.LEASE_EXPIRY:
        return `SmartStartPM: Your lease expires in ${data.data?.daysUntilExpiry} days. Contact your property manager for renewal.`;
      default:
        return `SmartStartPM: ${data.message}`;
    }
  }
