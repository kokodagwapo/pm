/**
 * SmartStartPM - Comprehensive Error Handling System
...
export class SmartStartPMError extends Error {
...
export function handleApiError(error: any): SmartStartPMError {
  // Network errors
  if (!navigator.onLine) {
    return new SmartStartPMError(ErrorType.NETWORK, "Network unavailable", {
  ...
  if (error instanceof TypeError && error.message.includes("fetch")) {
    return new SmartStartPMError(ErrorType.NETWORK, "Network request failed", {
  ...
  if (error?.status) {
    switch (error.status) {
      case 400:
        return new SmartStartPMError(ErrorType.VALIDATION, "Invalid request", {
          code: "VALIDATION_REQUIRED_FIELD",
          details: error.data,
        });
      case 401:
        return new SmartStartPMError(
      ...
      case 403:
        return new SmartStartPMError(ErrorType.AUTHORIZATION, "Access denied", {
          code: "AUTH_ACCOUNT_LOCKED",
        });
      case 404:
        return new SmartStartPMError(ErrorType.DATABASE, "Resource not found", {
      ...
      case 500:
        return new SmartStartPMError(ErrorType.DATABASE, "Server error", {
          code: "NETWORK_SERVER_ERROR",
          retryable: true,
        });
      default:
        return new SmartStartPMError(ErrorType.UNKNOWN, "Unknown error", {
          code: "UNKNOWN_ERROR",
          retryable: true,
        });
    }
...
export function handlePaymentError(error: any): SmartStartPMError {
  const errorCode = error?.code || error?.type;

  switch (errorCode) {
    case "card_declined":
      return new SmartStartPMError(ErrorType.PAYMENT, "Payment declined", {
        code: "PAYMENT_DECLINED",
        retryable: true,
      });
    case "insufficient_funds":
      return new SmartStartPMError(ErrorType.PAYMENT, "Insufficient funds", {
        code: "PAYMENT_INSUFFICIENT_FUNDS",
      });
    case "expired_card":
      return new SmartStartPMError(ErrorType.PAYMENT, "Card expired", {
        code: "PAYMENT_EXPIRED_CARD",
      });
    case "invalid_card":
      return new SmartStartPMError(ErrorType.PAYMENT, "Invalid card", {
...
export function showErrorToast(error: any) {
  const appError =
    error instanceof SmartStartPMError ? error : handleApiError(error);
